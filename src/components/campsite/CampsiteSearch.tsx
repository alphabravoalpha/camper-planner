...
export interface CampsiteSearchProps {
  ...
  maxResults?: number;
  placeholder?: string;
  mapInstance?: L.Map; // Optional map instance for auto-zoom functionality
}
...
const CampsiteSearchInner: React.FC<CampsiteSearchProps> = ({
  ...
  const performSearch = useCallback(
    async (searchQuery: string) => {
      ...
      let searchBounds: { north: number; south: number; east: number; west: number } | null = null;
      let searchRequest: CampsiteRequest;

      // Check if query looks like a location name (not a specific campsite name)
      const isLocationQuery =
        !searchQuery.toLowerCase().includes('camp') &&
        ...
        if (isLocationQuery) {
          ...
          if (lastGeocodedQuery.current !== searchQuery.trim()) {
            try {
              ...
              // Geocoding failed - continue with fallback search
            }
          } else {
          ...
          if (map && mapCenter) {
            try {
              ...
              // Failed to pan map - non-critical
            }
          }
        } else {
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- performSearch and onSearchChange intentionally omitted to prevent infinite re-render loops
  }, [query]);
  ...
  const handleCampsiteSelect = useCallback(
    (campsite: SearchResult) => {
    ...
    <div className={cn('relative', className)}>
      ...
      {showResults &&
        (searchResults.length > 0 ||
          ...
          (isLocationQuery && query.trim().length >= 3)) && (
          <div
            ref={dropdownRef}
            role="listbox"
            tabIndex={-1}
            ...
            {isLocationQuery &&
              ...
              query.trim().length >= 3 && (
                <div className="px-3 py-4 text-center">
                  ...
                  <div className="text-xs text-neutral-400">
                    {map?.getCenter() && map.getCenter().lat !== 54.526 ? (
                      <span>
                        Campsite data may be temporarily unavailable. The map has been zoomed to
                        your searched location.
                      </span>
                    ) : (
                      <span>
                        Try a different location or zoom out on the map to search a larger area
                      </span>
                    )}
                  </div>
                </div>
                ...
                {searchHistory.slice(0, 5).map((historyItem, _index) => (
                  ...
                  >
                    <div className="flex items-center">
                      <svg
                        className="h-3 w-3 text-neutral-400 mr-2"
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                      >
