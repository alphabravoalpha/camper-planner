      ...
      {showTools && (
        <div className="bg-white border-t border-neutral-200 px-3 py-2 flex gap-2 animate-fade-in">
          ...
          <button
            ...
            className={cn(
              "flex-1 flex items-center justify-center gap-1.5 py-2 px-3 rounded-lg text-xs font-medium transition-colors",
              showTripManager ? "bg-indigo-100 text-indigo-700" : "bg-neutral-100 text-neutral-700 active:bg-neutral-200"
            )}
          >
          ...
          <button
            onClick={() => { onTogglePlanningTools(); setShowTools(false); }}
            className={cn(
              "flex-1 flex items-center justify-center gap-1.5 py-2 px-3 rounded-lg text-xs font-medium transition-colors",
              showPlanningTools ? "bg-violet-100 text-violet-700" : "bg-neutral-100 text-neutral-700 active:bg-neutral-200"
            )}
          ...
          <button
            onClick={() => { onToggleCostCalculator(); setShowTools(false); }}
            className={cn(
              "flex-1 flex items-center justify-center gap-1.5 py-2 px-3 rounded-lg text-xs font-medium transition-colors",
              showCostCalculator ? "bg-emerald-100 text-emerald-700" : "bg-neutral-100 text-neutral-700 active:bg-neutral-200"
            )}
            disabled={waypointCount < 2}
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="flex items-center gap-0.5">
            ...
            <button
              ...
              className={cn(
                ...
                campsiteActive ? "bg-green-100 text-green-700" : "text-neutral-600 active:bg-neutral-100"
              )}
            ...
            {hasRoute && (
              <button
                ...
                className={cn(
                  "w-10 h-10 flex items-center justify-center rounded-lg transition-colors",
                  showRouteInfo ? "bg-primary-100 text-primary-700" : "text-neutral-600 active:bg-neutral-100"
                )}
