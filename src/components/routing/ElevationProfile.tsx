...
const ElevationProfile: React.FC<ElevationProfileProps> = ({ route, className }) => {
  ...
  const xScale = (distance: number) =>
    (distance / stats.totalDistance / 1000) * CHART_WIDTH;

  const yScale = (elevation: number) =>
    CHART_HEIGHT - ((elevation - stats.minElevation) / (stats.maxElevation - stats.minElevation)) * CHART_HEIGHT;
          ...
          >
            ...
            <g stroke="#e5e7eb" strokeWidth="1">
              ...
              {[0, 0.25, 0.5, 0.75, 1].map(ratio => {
                ...
                const elevation = stats.maxElevation - ratio * (stats.maxElevation - stats.minElevation);
                return (
                  <g key={ratio}>
                    <line
                      x1={MARGIN.left}
