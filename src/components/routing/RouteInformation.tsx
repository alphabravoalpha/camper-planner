...
const RouteSummary: React.FC<{ route: RouteResponse; compact?: boolean }> = ({
  route,
  compact,
}) => {
    ...
    <div className="bg-white rounded-lg border border-neutral-200 p-4">
      ...
      {profile && (
        <div className="mt-3 text-xs text-neutral-600">
          Vehicle: {profile.height}m H × {profile.width}m W × {profile.weight}t × {profile.length}m
          L
        </div>
      )}
    </div>
        ...
        <nav className="-mb-px flex space-x-8 px-4">
          {[
            ...
            <button
              key={tab.id}
              onClick={() =>
                setActiveTab(tab.id as 'summary' | 'segments' | 'directions' | 'elevation')
              }
              className={cn(
