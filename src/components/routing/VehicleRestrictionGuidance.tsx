  ...
  const getRestrictionAdvice = () => {
    ...
    if (restrictions.cannotAccommodate) {
      advice.push({
        ...
        description: 'Your vehicle dimensions exceed EU road limits. Consider reviewing vehicle specifications.',
        actions: [
      ...
      <div className="flex items-start space-x-3 mb-4">
        ...
        <div className="flex-1">
          ...
          <p className={cn(
            ...
            {waypoints.length} waypoints • {restrictions.violatedDimensions.length} dimension{restrictions.violatedDimensions.length > 1 ? 's' : ''} exceed EU limits
          </p>
        </div>
      </div>
        ...
        <div className="mb-4 p-3 bg-white rounded border">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div className={cn(
              ...
              <span>Height:</span>
              <span>{profile.height}m {restrictions.violatedDimensions.includes('height') && '⚠️'}</span>
            </div>
            <div className={cn(
              'flex justify-between',
              ...
              <span>Width:</span>
              <span>{profile.width}m {restrictions.violatedDimensions.includes('width') && '⚠️'}</span>
            </div>
            <div className={cn(
              'flex justify-between',
              ...
              <span>Weight:</span>
              <span>{profile.weight}t {restrictions.violatedDimensions.includes('weight') && '⚠️'}</span>
            </div>
            <div className={cn(
              'flex justify-between',
              ...
              <span>Length:</span>
              <span>{profile.length}m {restrictions.violatedDimensions.includes('length') && '⚠️'}</span>
            </div>
          </div>
        </div>
      )}
        ...
        <div key={index} className="mb-4">
          ...
          <ul className="space-y-1">
            {adviceItem.actions.map((action, actionIndex) => (
              <li key={actionIndex} className={cn(
