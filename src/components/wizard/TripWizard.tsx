...
const STEPS = [
  { id: 'start-end', labelKey: 'wizard.steps.route', icon: MapPin },
  { id: 'dates', labelKey: 'wizard.steps.dates', icon: Calendar },
  { id: 'driving', labelKey: 'wizard.steps.driving', icon: Car },
  { id: 'crossing', labelKey: 'wizard.steps.crossing', icon: Ship },
  { id: 'itinerary', labelKey: 'wizard.steps.itinerary', icon: Route },
];
    ...
    <div className="fixed inset-0 z-[9999] bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl ring-1 ring-black/5 w-full max-w-3xl max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-neutral-100">
          <h2 className="text-xl font-display font-bold text-neutral-900">{t('wizard.title')}</h2>
          <button
            onClick={() => {
      ...
      <ConfirmDialog
        ...
        title={t('wizard.closeConfirm.title')}
        message={t('wizard.closeConfirm.message')}
        confirmLabel={t('wizard.closeConfirm.confirm')}
        cancelLabel={t('wizard.closeConfirm.cancel')}
      ...
      <ConfirmDialog
        ...
        title={t('wizard.overwriteConfirm.title')}
        message={t('wizard.overwriteConfirm.message')}
        confirmLabel={t('wizard.overwriteConfirm.confirm')}
...
const StepStartEnd: React.FC = () => {
  ...
  const searchLocation = useCallback(
    async (query: string): Promise<GeocodeMappedResult[]> => {
      ...
      try {
        ...
        setSearchError(t('wizard.searchError'));
        return [];
      }
    },
    [t]
    ...
    <div className="space-y-6">
      <div>
        <h3 className="text-lg font-display font-semibold text-neutral-900 mb-1">
          {t('wizard.startEnd.title')}
        </h3>
        <p className="text-sm text-neutral-500">{t('wizard.startEnd.description')}</p>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <div className="relative">
            <input
              ...
              placeholder={t('wizard.startEnd.startPlaceholder')}
        ...
        <div className="flex justify-center">
          <button
            ...
            title={t('wizard.startEnd.swapTitle')}
          >
        ...
        <div>
          ...
          <div className="relative">
            <input
              ...
              placeholder={t('wizard.startEnd.endPlaceholder')}
              className="w-full px-4 py-3 border-2 border-neutral-200 rounded-lg focus:ring-2 focus:ring-primary-200 focus:border-primary-500 text-base"
            />
            {searchingEnd && (
              <Loader2 className="absolute right-3 top-3.5 w-5 h-5 text-neutral-400 animate-spin" />
      ...
      {start && end && (
        <div className="bg-primary-50 rounded-xl p-4 text-sm text-primary-800">
          ...
          <span className="text-primary-600">
            {t('wizard.startEnd.approxDistance', {
              km: Math.round(
                6371 *
                  2 *
                  Math.atan2(
...
const StepDates: React.FC = () => {
  ...
  const getSeason = (date: Date): { name: string; icon: React.ReactNode; tip: string } => {
    ...
    if (month >= 2 && month <= 4)
      return {
        name: t('wizard.dates.spring'),
        icon: <Leaf className="w-5 h-5 text-green-500" />,
        tip: t('wizard.dates.springTip'),
      };
    if (month >= 5 && month <= 7)
      return {
        name: t('wizard.dates.summer'),
        icon: <Sun className="w-5 h-5 text-yellow-500" />,
        tip: t('wizard.dates.summerTip'),
      };
    if (month >= 8 && month <= 10)
      return {
        name: t('wizard.dates.autumn'),
        icon: <CloudRain className="w-5 h-5 text-orange-500" />,
        tip: t('wizard.dates.autumnTip'),
      };
    return {
      name: t('wizard.dates.winter'),
      icon: <Snowflake className="w-5 h-5 text-blue-400" />,
      tip: t('wizard.dates.winterTip'),
    };
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-neutral-500">{t('wizard.dates.description')}</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
          <label
            ...
            className="block text-sm font-semibold text-neutral-700 mb-1"
          >
            {t('wizard.dates.returnLabel')}{' '}
            <span className="text-neutral-400">{t('wizard.dates.optional')}</span>
          </label>
          <input
        ...
        <div className="flex items-start gap-3 bg-amber-50 rounded-lg p-4">
          ...
          <div>
            <p className="font-medium text-amber-900">
              {t('wizard.dates.seasonTrip', { season: season.name })}
            </p>
...
const StepDrivingStyle: React.FC = () => {
  ...
  const styles: { id: DrivingStyle; icon: React.FC<{ className?: string }>; titleKey: string }[] = [
    { id: 'relaxed', icon: Sun, titleKey: 'wizard.driving.relaxed' },
    { id: 'moderate', icon: Scale, titleKey: 'wizard.driving.moderate' },
    { id: 'intensive', icon: Rocket, titleKey: 'wizard.driving.intensive' },
  ];
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-lg font-display font-semibold text-neutral-900 mb-1">
          {t('wizard.driving.title')}
        </h3>
        <p className="text-sm text-neutral-500">{t('wizard.driving.description')}</p>
      </div>
        ...
        {styles.map(style => {
          ...
          return (
            ...
            >
              ...
              <h4 className="font-display font-semibold text-neutral-900">{t(style.titleKey)}</h4>
      ...
      <div>
        <span id="rest-days-label" className="block text-sm font-semibold text-neutral-700 mb-2">
          {t('wizard.driving.restDaysLabel')}
        </span>
        <div className="flex gap-2" role="group" aria-labelledby="rest-days-label">
          {[
            { value: 0, label: t('wizard.driving.noRestDays') },
            { value: 3, label: t('wizard.driving.every3Days') },
            { value: 5, label: t('wizard.driving.every5Days') },
          ].map(option => (
            <button
              key={option.value}
      ...
      {profile && (
        <div className="bg-neutral-50 rounded-xl p-3 text-sm text-neutral-600">
          ...
          {t('wizard.driving.yourVehicle', { name: profile.name || profile.type || 'Custom' })}
          {profile.type === 'motorhome' && drivingStyle === 'intensive' && (
            <span className="text-amber-600 ml-2">{t('wizard.driving.motorhomeWarning')}</span>
          )}
        </div>
      )}
    </div>
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-4 mt-1.5 text-xs text-neutral-500">
        ...
        {c.overnightCrossing && (
          <span className="text-purple-600">{t('wizard.crossing.overnight')}</span>
        )}
      </div>
      ...
      {isSelected && c.bookingUrls.length > 0 && (
        <div className="flex gap-2 mt-2">
          {c.bookingUrls.map((url, i) => (
            ...
            >
              {t('wizard.crossing.bookWith', { operator: c.operators[i] || c.operators[0] })}
...
const StepCrossing: React.FC<StepCrossingProps> = ({ detected }) => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-lg font-display font-semibold text-neutral-900 mb-1">
          {t('wizard.crossing.title')}
        </h3>
        <p className="text-sm text-neutral-500">
          {hasMandatory
            ? t('wizard.crossing.mandatoryDesc', { count: mandatory.length })
            : t('wizard.crossing.optionalDesc')}
        </p>
      </div>
        ...
        <div className="space-y-4">
          {mandatory.length > 1 && (
            <h4 className="font-display font-semibold text-neutral-800 text-sm uppercase tracking-wide">
              {t('wizard.crossing.requiredCrossings')}
            </h4>
          )}
            ...
            return (
              <div key={`mandatory-${idx}`} className="space-y-2">
                ...
                {segment.crossings.length > 1 && (
                  ...
                  >
                    {isExpanded
                      ? t('wizard.crossing.hideAlternatives')
                      : t('wizard.crossing.alternativesAvailable', {
                          count: segment.crossings.length - 1,
                        })}
                  </button>
                )}
      ...
      {hasOptional && (
        <div className="space-y-3">
          <h4 className="font-display font-semibold text-neutral-800 text-sm uppercase tracking-wide">
            {t('wizard.crossing.optionalFerries')}
          </h4>
          <p className="text-xs text-neutral-500">{t('wizard.crossing.optionalFerriesDesc')}</p>
          {optional.map(({ crossing: c, estimatedSavingsKm }) => (
            <div key={c.id} className="relative">
              ...
              <span className="absolute top-3 right-12 text-xs bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full">
                {t('wizard.crossing.estimatedSavings', { km: estimatedSavingsKm })}
              </span>
            </div>
          ))}
        </div>
    ...
    return (
      <div className="flex flex-col items-center justify-center py-16">
        ...
        <h3 className="text-lg font-display font-semibold text-neutral-900">
          {t('wizard.itinerary.generatingTitle')}
        </h3>
        <p className="text-sm text-neutral-500 mt-1">{t('wizard.itinerary.generatingDesc')}</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-display font-bold text-neutral-900">
            {t('wizard.itinerary.title')}
          </h3>
          <p className="text-sm text-neutral-500">
            {t('wizard.itinerary.summary', {
              days: itinerary.totalDays,
              distance: itinerary.totalDistance,
              time: itinerary.totalDrivingTime,
            })}
          </p>
        </div>
      </div>
      ...
      <div className="sticky bottom-0 bg-white pt-4 pb-2 border-t border-neutral-100 -mx-6 px-6">
        ...
        >
          {t('wizard.itinerary.createButton')}
        </button>
      </div>
    </div>
  );
      ...
      <button onClick={() => setExpanded(!expanded)} className="w-full text-left p-4">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="font-medium text-neutral-900">
                  {isRest
                    ? t('wizard.itinerary.restDay', { name: day.start.name })
                    : t('wizard.itinerary.dayRoute', { from: day.start.name, to: day.end.name })}
                </span>
                {day.type === 'crossing' && (
                  <span className="text-xs bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full">
                    <Ship className="w-3 h-3 inline mr-0.5" />
                    {t('wizard.itinerary.crossingBadge')}
                  </span>
                )}
              </div>
              <div className="text-xs text-neutral-500 mt-0.5">
                {dateStr}
                {isDriving &&
                  ` · ${t('wizard.itinerary.dayStats', { distance: day.distance, time: day.drivingTime })}`}
              </div>
            </div>
          </div>
          <ChevronRight
        ...
        {day.selectedOvernight && !expanded && (
          <div className="flex items-center gap-2 mt-2 ml-11 text-xs text-neutral-500">
            ...
            <span>
              {t('wizard.itinerary.stayingAt')}{' '}
              <strong>{day.selectedOvernight.campsite.name}</strong>
            </span>
          ...
          {day.crossing && (
            <div className="ml-11 bg-blue-50 rounded-lg p-3 text-sm">
              ...
              <p className="text-xs text-blue-700 mt-1">
                ...
                {t('wizard.itinerary.crossingInfo')} ·{' '}
                {day.crossing.estimatedCost.currency === 'EUR' ? '\u20AC' : '\u00A3'}
              ...
              <div className="flex gap-2 mt-2">
                {day.crossing.bookingUrls.map((url: string, i: number) => (
                  ...
                  >
                    {t('wizard.itinerary.bookOperator', {
                      operator: day.crossing.operators[i] || day.crossing.operators[0],
                    })}
                    <ExternalLink className="w-3 h-3" />
                  </a>
                ))}
          ...
          {day.overnightOptions.length === 0 && (
            <div className="ml-11 p-3 bg-neutral-50 rounded-lg border border-neutral-200">
              <p className="text-sm text-neutral-500 flex items-center gap-2">
                ...
                {t('wizard.itinerary.noCampsites')}
              </p>
            </div>
          )}
          {day.overnightOptions.length > 0 && (
            <div className="ml-11">
              <p className="text-xs font-medium text-neutral-700 mb-2">
                ...
                {t('wizard.itinerary.overnightOptions', { count: day.overnightOptions.length })}
              </p>
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <span className="text-xs text-neutral-500 ml-2">
                            {t('wizard.itinerary.kmFromRoute', { km: option.distanceFromRoute })}
                          </span>
                        </div>
